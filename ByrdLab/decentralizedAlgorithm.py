...
from ByrdLab.library.measurements import avg_loss_accuracy_dist, consensus_error, one_node_loss_accuracy_dist, avg_model_loss_accuracy_dist
from ByrdLab.library.tool import flatten_list, log
    ...
    def run(self):
        ...
        for iteration in range(0, self.total_iterations + 1):
            ...
            if iteration % self.display_interval == 0:
                ...
                test_loss, test_accuracy = avg_model_loss_accuracy_dist(
                    dist_models, self.get_test_iter,
                    self.loss_fn, self.test_fn,
                    weight_decay=0, node_list=self.honest_nodes)
...
# alternative: Apatation & Combination
class DSGD_under_DPA(Dec_Byz_Opt_Env):
    """_summary_

    Args:
        Dec_Byz_Opt_Env (): environment for decentralized optimization with Byzantine attacks.
    """
    def __init__(self, graph, aggregation, consensus_init=False, *args, **kw):
        super().__init__(name='DSGD', graph=graph, *args, **kw)
        self.consensus_init = consensus_init
    ...
    def run(self):
        ...
        for iteration in range(0, self.total_iterations + 1):
            ...
            for node in self.nodes:
                ...
                aggregation_res = self.aggregation.run(param_bf_comm, node)
                dist_models.params_vec[node].copy_(aggregation_res)
            #     if self.aggregation.name != 'meanW':
            #         if iteration % self.display_interval == 0 and node in self.graph.honest_nodes:
            #             rho_w = self.aggregation.get_rhow(param_bf_comm, node)
            #             rho = max(rho, rho_w)

            # if self.aggregation.name != 'meanW':
            #     if iteration % self.display_interval == 0:
            #         lamda, beta = self.aggregation.get_lambda_beta()
            #         print(f'rho: {rho:.3f} lamda: {lamda:.3f}, beta: {beta:.3f}')
...
class DSGD_with_LFighter_under_DPA(Dec_Byz_Opt_Env):
    def __init__(self, graph, aggregation, consensus_init=False, *args, **kw):
        super().__init__(name='DSGD', graph=graph, *args, **kw)
        self.consensus_init = consensus_init
        self.aggregation = aggregation
    ...
    def run(self):
        ...
        # local models before communication
        param_bf_comm = [
            [torch.zeros_like(para, requires_grad=False) for para in dist_models.model.parameters()]
        ...
        for iteration in range(0, self.total_iterations + 1):
            ...
            for node in self.nodes:
                ...
                # store the model's paramaters before communication
                for index, para in enumerate(model.parameters()):
                    param_bf_comm[node][index].data.zero_()
                    param_bf_comm[node][index].data.add_(para.data, alpha=1)
...
# alternative: Apatation & Combination
class DSGD_compute_hetero_disturbance(Dec_Byz_Opt_Env):
    def __init__(self, graph, aggregation, consensus_init=False, *args, **kw):
        super().__init__(name='DSGD', graph=graph, *args, **kw)
        self.consensus_init = consensus_init
        self.aggregation = aggregation
    ...
    def run(self):
        ...
        for iteration in range(0, self.total_iterations + 1):
            ...
            if iteration % self.display_interval == 0:
                ...
                # Compute heterogeneity and disturbance on the avg model in each display interval
                for node in self.nodes:
                    
                    features, targets = next(data_iters[node])
                    if node in self.byzantine_nodes:
                        features, targets = self.attack.run(features, targets, model=avg_model)

                    features = features.to(DEVICE)
                    targets = targets.to(DEVICE)
                    predictions = avg_model(features)
                    loss = self.loss_fn(predictions, targets)
                    avg_model.zero_grad()
                    loss.backward()

                    for index, para in enumerate(avg_model.parameters()):
                        local_grads[node][index].data.zero_()
                        local_grads[node][index].data.add_(para.grad.data, alpha=1)
                
                local_grads_flat = flatten_list(local_grads)
                global_grad_flat = torch.mean(local_grads_flat[self.honest_nodes], dim=0)
                distances_A = torch.tensor([torch.norm(local_grads_flat[node] - global_grad_flat) for node in self.byzantine_nodes])
                distances_hetero = torch.tensor([torch.norm(local_grads_flat[node] - global_grad_flat) for node in self.honest_nodes])
                disturbance_A_max = distances_A.max()
                hetero_max = distances_hetero.max()
                print('disturbance_A:', disturbance_A_max.item())
                print('hetero:', hetero_max.item())
                disturbance_A_list.append(disturbance_A_max.item())
        ...
        dist_models.activate_avg_model()
        avg_model = dist_models.model
        return avg_model, loss_path, acc_path, consensus_error_path, disturbance_A_list, hetero_list


# alternative: Apatation & Combination
# class DSGD_compute_hetero_disturbance_2(Dec_Byz_Opt_Env):
#     def __init__(self, graph, aggregation, consensus_init=False, *args, **kw):
#         super().__init__(name='DSGD', graph=graph, *args, **kw)
#         self.consensus_init = consensus_init
#         self.aggregation = aggregation
