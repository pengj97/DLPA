...
from ByrdLab.aggregation import (D_bulyan, D_faba, D_geometric_median, D_Krum, 
                                 ...
                                 D_mKrum, D_centered_clipping, D_ios, D_brute, D_mean, D_LFighter)
...
from ByrdLab.decentralizedAlgorithm import DSGD, DSGD_under_DPA, DSGD_MSG, DSGD_MSG_under_DPA, DSGD_with_LFighter_under_DPA, DSGD_compute_hetero_disturbance
from ByrdLab.graph import CompleteGraph, ErdosRenyi, OctopusGraph, TwoCastle, LineGraph, RandomGeometricGraph, RingGraph,\
                          Graph_byz_nodes_on_shortest_paths, Graph_byz_nodes_Not_on_shortest_paths, StarGraph, RegularCompleteGraph, TwoHeadLineGraph, FanGraph
...
if args.graph == 'Complete':
    ...
    graph = RegularCompleteGraph(node_size=10, byzantine_size=5)
...
task = ResNetTask(data_package, batch_size=5000)

# data_package = cifar100()
# task = ResNetTask(data_package, batch_size=1000)
...
env = DSGD_compute_hetero_disturbance(aggregation=aggregation, graph=graph, attack=attack, step_agg = step_agg,
...
log('[Start Running]')
_, loss_path, acc_path, consensus_error_path, disturbance_list, hetero_list = env.run()
...
if record_in_file:
    ...
    dump_file_in_cache(title + f'_disturbance_A_seed={seed}', disturbance_list, path_list=path_list)
    dump_file_in_cache(title + f'_hetero_list_seed={seed}', hetero_list, path_list=path_list)
print('-------------------------------------------')
